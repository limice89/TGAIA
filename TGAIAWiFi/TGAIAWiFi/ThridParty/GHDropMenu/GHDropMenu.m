...
- (void)show {
    ...
    [UIView animateWithDuration:self.durationTime animations:^{
        if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle /** 普通菜单 */) {
            CGFloat heights = (dropMenuTitleModel.dataArray.count * self.cellHeight>(kGHScreenHeight - self.menuHeight - kGHSafeAreaTopHeight-kSafeArea_Bottom))?(kGHScreenHeight - self.menuHeight - kGHSafeAreaTopHeight-kSafeArea_Bottom):dropMenuTitleModel.dataArray.count * self.cellHeight;
            self.tableView.frame = CGRectMake(0, self.tableY, self.frame.size.width, heights);
            self.titleCover.frame = CGRectMake(0, self.tableY, kGHScreenWidth, kGHScreenHeight - self.menuHeight - kGHSafeAreaTopHeight-kSafeArea_Bottom);
