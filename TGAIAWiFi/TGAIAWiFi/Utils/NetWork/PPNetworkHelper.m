...
//        [MBProgressHUD hideHUD];
        if (_isOpenLog) {PPLog(@"URL = %@ responseObject = %@",URL,[responseObject modelToJSONString]);}
        [[self allSessionTask] removeObject:task];
        if (success) {
            [IECNetErrorManager handleReqRespouse:responseObject successBlock:^(id  _Nonnull detail, NSString * _Nonnull msg) {
                success(detail);
            } failBlock:^(NSString * _Nonnull errorMsg) {
                failure ? failure(errorMsg) : nil;
            }];
        }
...
//        [MBProgressHUD hideHUD];
        ...
        if ([error.domain isEqualToString:AFURLResponseSerializationErrorDomain]) {
            ...
            NSString * code = NSStringFormat(@"%@",[response objectForKey:@"status"]);
            NSString * errormsg = [response objectForKey:@"msg"];
            if (([code isEqualToString:@"401"]&&[errormsg containsString:@"token"])||([code isEqualToString:@"401"]&&[errormsg containsString:@"登录状态失效"])){
                [userManager clearUserInfo];
                KPostNotification(KNotificationBackToLogin, nil);
            }
            message = [NSSTRING_CHECK_ISNULL(errormessage) isEqualToString:@""]?errormsg:errormessage;
            // response中包含服务端返回的内容
...
//        [MBProgressHUD hideHUD];
        if (_isOpenLog) {PPLog(@"URL = %@ responseObject = %@",URL,responseObject);}
        [[self allSessionTask] removeObject:task];
            if (success) {
                [IECNetErrorManager handleReqRespouse:responseObject successBlock:^(id  _Nonnull detail, NSString * _Nonnull msg) {
                    success(detail);
...
//        [MBProgressHUD hideHUD];
        ...
        if ([error.domain isEqualToString:AFURLResponseSerializationErrorDomain]) {
            ...
            message = [NSSTRING_CHECK_ISNULL(errormsg) isEqualToString:@""]?message:errormsg;
            // response中包含服务端返回的内容
        } else if ([error.domain isEqualToString:NSCocoaErrorDomain]) {
            // server throw exception
            message = error.localizedDescription;
...
+ (NSURLSessionTask *)downloadWithURL:(NSString *)URL
    ...
    __block NSURLSessionDownloadTask *downloadTask = [_sessionManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if(failure && error) {failure(error.localizedDescription) ; return ;};
...
+ (NSURLSessionTask *)DELETE:(NSString *)URL
                parameters:(id)parameters
                   ...
                   failure:(PPHttpRequestFailed)failure {
//    [MBProgressHUD showActivityMessageInView:@""];
    if ((parameters!=nil && [parameters isKindOfClass:[NSArray class]] && [parameters count]>=0)) {
        parameters = [NSJSONSerialization dataWithJSONObject:parameters options:NSJSONWritingPrettyPrinted  error:nil];
    }
...
//        [PPNetworkHelper setValue:NSStringFormat(@"%@%@",@"Bearer",userManager.curUserInfo.access_token) forHTTPHeaderField:@"Authorization"];
//    };
}
@end
